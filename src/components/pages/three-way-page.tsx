  ...
  return (
    <div className="bg-[#FAFAFA]">
      ...
      <div className="mx-auto max-w-[1100px] px-4 pb-16 pt-10 sm:px-6">
          ...
          {providers.map((p) => (
            <div key={p.id} className="flex flex-col rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <ProviderCta
                ...
                className="flex h-[42px] items-center justify-center gap-1.5 rounded-xl bg-[#0C4B75] text-[13.5px] font-bold text-white transition-colors hover:bg-[#093d61]"
              >
                Visit {p.name}
                ...
                {MATRIX_ROWS.map((row) => (
                  <tr key={row.label} className="border-b border-gray-100 align-top last:border-0">
                    ...
                    {trio.providerIds.map((id) => (
                      <td key={id} className="break-words py-3.5 px-3 text-[13px] leading-snug text-[#191919] sm:text-[14px]">
        ...
        <div className="mb-12">
          <h2 className="mb-5 text-[22px] font-bold text-[#191919]">{trio.title.split(":")[0]}: FAQs</h2>
          <div className="space-y-4">
            {trio.faqs.map((f, i) => (
              <div key={i} className="rounded-xl border border-gray-200 bg-white p-5">
                <p className="mb-2 flex items-start gap-2 text-[15px] font-semibold text-[#191919]">
        ...
        {relatedBattles.length > 0 && (
          <div className="mb-4">
            <h2 className="mb-1.5 text-[20px] font-bold text-[#191919]">Break it down two at a time</h2>
